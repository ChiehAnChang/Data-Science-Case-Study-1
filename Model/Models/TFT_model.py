...
class TFTModel(BasePM25Model):
    ...
    def _to_timeseries(self, df: pd.DataFrame):
        ...
        cov_ts = TimeSeries.from_dataframe(df_cov, value_cols=cov_cols)
        return target_ts, cov_ts

    def fit(self, train_df: pd.DataFrame):
        target_ts, cov_ts = self._to_timeseries(train_df)
        target_scaled = self.scaler.fit_transform(target_ts)
        cov_scaled = self.cov_scaler.fit_transform(cov_ts)
        
        self.model = DartsTFT(
            input_chunk_length=self.input_chunk_length,
            output_chunk_length=self.horizon, # 🚀 Darts 輸出對齊 Horizon
            hidden_size=16, lstm_layers=1, num_attention_heads=1, dropout=0.1, batch_size=64, n_epochs=10,            
            likelihood=QuantileRegression(quantiles=[0.25, 0.5, 0.75]),
            add_relative_index=True, random_state=2026,
            pl_trainer_kwargs={"accelerator": "gpu", "devices": 1}
        )
        self.model.fit(series=target_scaled, past_covariates=cov_scaled, verbose=False)
        self.is_fitted = True
